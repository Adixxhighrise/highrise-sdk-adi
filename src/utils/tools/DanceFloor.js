'use strict';
const Emotes = require("../Emotes"); // Adjust the path if necessary
...
module.exports = class DanceFloor {
  constructor(bot, options = {}) {
    ...
    this.playersOnDanceFloor = new Set(); // Track players on the dance floor
    setInterval(async () => {
  ...
  async create() {
    try {
      ...
      for (const group of groups) {
        const randomEmoteKey = Object.keys(this.emotes)[Math.floor(Math.random() * Object.keys(this.emotes).length)];
        const randomEmote = this.emotes[randomEmoteKey];
        const emoteDuration = randomEmote.duration * 1000;

        await this.performEmotes(group, randomEmote.id, emoteDuration);
      }

      console.log("[i] Dance floor created with " + players.length + " players!");
    } catch (error) {
  ...
  async performEmotes(group, emoteId, emoteDuration) {
    ...
    return new Promise(resolve => setTimeout(resolve, emoteDuration));
  }

  addPlayer(playerId) {
    this.playersOnDanceFloor.add(playerId);
  }

  removePlayer(playerId) {
    this.playersOnDanceFloor.delete(playerId);
  }
